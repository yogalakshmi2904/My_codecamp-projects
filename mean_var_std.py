...
def calculate(list):
    ...
    return{
  'mean':[mean_rows, mean_cols,ls.mean()],
  'variance': [var_rows, var_cols, ls.var()],
  'standard deviation': [std_rows, std_cols, ls.std()],
  'max': [max_rows, max_cols, ls.max()],
  'min': [min_rows, min_cols, ls.min()],
  'sum': [sum_rows, sum_cols, ls.sum()]
}